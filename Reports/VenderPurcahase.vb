...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(VenderPurcahase))
...
#End Region
    ...
    Private Sub Cmd_View_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_View.Click
        If chklst_Group.CheckedItems.Count = 0 Then
            MessageBox.Show("Select the Category Name(s)", MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
    ...
    Private Sub Timer1_Tick(ByVal sender As Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        Else
            ...
            If MsgBox("Click YES for 'Windows View' or NO for 'TEXT View'", MsgBoxStyle.YesNo, "Purchase Order") = MsgBoxResult.Yes Then
    ...
    Private Sub Viewsuppliernamewise_crystal()
        Try
            ...
            Dim RT As New Cry_vendorPurchase




            'sqlstring = "select * from VW_POPRINT_RPT where VENDORCODE in( "
            'If chklst_Supplier.CheckedItems.Count <> 0 Then
            '    'sqlstring = sqlstring & " And VENDORCODE IN ("
            '    For i = 0 To chklst_Supplier.CheckedItems.Count - 1
            '        DEPARTMENT = Split(chklst_Supplier.CheckedItems(i), "-->")
            ...
            'sqlstring = sqlstring & " AND PODATE BETWEEN"
            'sqlstring = sqlstring & " '" & Format(dtp_Fromdate.Value, "dd-MMM-yyyy") & "' AND ' " & Format(dtp_Todate.Value, "dd-MMM-yyyy") & "'"
            'sqlstring = sqlstring & " ORDER BY PODATE"
            'gconnection.getDataSet(sqlstring, "VW_POPRINT_RPT")
            ...
            '    rViewer.TableName = "VW_POPRINT_RPT"
            ...
            '    'T2.Text = "Tin No.:" & gTinNo & ",Gstin No:" & gServiceTax
            '    rViewer.Show()
            ...
            sqlstring = "select * from VW_POPRINT_RPT where SUPPLIERCODE in( "
            If chklst_Supplier.CheckedItems.Count <> 0 Then

                For i = 0 To chklst_Supplier.CheckedItems.Count - 1
                    DEPARTMENT = Split(chklst_Supplier.CheckedItems(i), "-->")
                    sup = sup & " '" & Trim(DEPARTMENT(0)) & "', "
                Next
                sup = Mid(sup, 1, Len(sup) - 2)

                sqlstring = sqlstring & sup & ") "
            Else
                MessageBox.Show("Select the Supplier Name(s)", MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If
            ...
            If dt.Rows.Count > 0 Then
                ...
                dt.TableName = "VW_POPRINT_RPT"
                ds.Tables.Add(dt)

                sqlstring1 = "select * from VW_PO_POBILL_DET where pono in (select distinct pono from VW_POPRINT_RPT where VENDORCODE in(" & sup & ")  AND PODATE BETWEEN '" & Format(dtp_Fromdate.Value, "dd-MMM-yyyy") & "' AND '" & Format(dtp_Todate.Value, "dd-MMM-yyyy") & "')"
                dt = gconnection.GetValues(sqlstring1)
                dt.TableName = "VW_PO_POBILL_DET"
                ds.Tables.Add(dt)
                'rViewer.ssql = sqlstring

                'rViewer.Report = RT
                ' rViewer.TableName = "VW_POPRINT_RPT"
                Call rViewer.GetDetailsDS(ds, RT)
                'Call rViewer.GetDetails1(sqlstring1, "VW_PO_POBILL_DET", RT)
                Dim textobj1 As TextObject
                textobj1 = RT.ReportDefinition.ReportObjects("Text13")
                textobj1.Text = MyCompanyName
                Dim TXTOBJ2 As TextObject
                TXTOBJ2 = RT.ReportDefinition.ReportObjects("Text30")
                TXTOBJ2.Text = " Prepared By : " & gUsername
                Dim TXTOBJ3 As TextObject
                'TXTOBJ3 = RT.ReportDefinition.ReportObjects("Text97")
                'TXTOBJ3.Text = " From  " & Format(dtp_Fromdate.Value, "dd/MM/yyyy") & "  To " & " " & Format(dtp_Todate.Value, "dd/MM/yyyy") & ""
                Dim T1 As TextObject
                T1 = RT.ReportDefinition.ReportObjects("Text5")
                T1.Text = Address1 & " , " & Address2 & " , " & gCity & " - " & gPincode
                Dim textobj7 As TextObject
                textobj7 = RT.ReportDefinition.ReportObjects("Text54")
                textobj7.Text = "Tel:" & GPHONE & " , Fax:" & gFax & ", Email:" & gEmail & ", Web:" & gWebsite
                Dim T2 As TextObject
                T2 = RT.ReportDefinition.ReportObjects("Text20")
                T2.Text = "Tin No.:" & gTinNo & ",Gstin No:" & gServiceTax
                rViewer.Show()
    ...
    Private Sub VenderPurcahase_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Fillgroup()
        ...
        sqlstring = "SELECT DISTINCT category FROM View_All_cate"
        'sqlstring = sqlstring + " union "
        'sqlstring = sqlstring + " select distinct Category from INVENTORYITEMMASTER"

        'sqlstring = "SELECT storecode,storedesc FROM STOREMASTER"
        gconnection.getDataSet(sqlstring, "View_All_cate")
        If gdataset.Tables("View_All_cate").Rows.Count - 1 >= 0 Then
            For i = 0 To gdataset.Tables("View_All_cate").Rows.Count - 1
                With gdataset.Tables("View_All_cate").Rows(i)
                    ...
                    chklst_Group.Items.Add(Trim(.Item("category")))
                End With
            Next i
        End If
    ...
    Private Sub Fillsuppliername()
        ...
        Dim m As Integer
        Dim cate(), selCat As String
        selCat = ""

        For m = 0 To chklst_Group.CheckedItems.Count - 1
            cate = Split(chklst_Group.CheckedItems(m), "--")
            selCat = selCat & "'" & cate(0) & "', "
        Next
        If Not String.IsNullOrEmpty(selCat) Then
            selCat = Mid(selCat, 1, Len(selCat) - 2)
            chklst_Supplier.Items.Clear()
        ...
        End If
        If chklst_Group.CheckedItems.Count <> 0 Then
            sqlstring = "SELECT ISNULL(VENDORCODE,0) AS VENDORCODE, ISNULL(VENDORNAME,'') AS VENDORNAME FROM PO_VIEW_VENDORMASTER where category IN (" & selCat & ") order by VENDORCODE "
    ...
    Private Sub chklst_Group_SelectedIndexChanged(sender As Object, e As EventArgs) Handles chklst_Group.SelectedIndexChanged
