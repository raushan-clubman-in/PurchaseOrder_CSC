Public Class UserAuthorization
    ...
    Private Sub GetStore()
        Dim i As Integer
        STORE.Items.Clear()
        gSQLString = "SELECT storecode AS storecode ,storedesc AS storedesc FROM STOREMASTER where isnull(Freeze,'' ) <> 'Y'"
        gconnection.getDataSet(gSQLString, "SUBLEDGERMASTER")
        If gdataset.Tables("SUBLEDGERMASTER").Rows.Count - 1 >= 0 Then
            For i = 0 To gdataset.Tables("SUBLEDGERMASTER").Rows.Count - 1
                With gdataset.Tables("SUBLEDGERMASTER").Rows(i)
                    STORE.Items.Add(Trim(.Item("storecode")) & "-->" & Trim(.Item("Storedesc")))
                End With
            Next i
        End If
        STORE.Sorted = True
    ...
    Private Sub cls1()
        ...
        dtp_todate.Visible = False
        load_desig()
        For i = 0 To STORE.Items.Count - 1
            STORE.SetItemChecked(i, False)
        Next
    End Sub
            ...
            If checkvalidation() = True Then
                ...
                If (LCase(btn_save.Text) = "save") Then
                    ...
                    Else
                        ORDERNO = 1
                    End If
                    If combo_service.Text = "Both" Then


                        For j = 0 To STORE.CheckedItems.Count - 1
                            Dim DEPARTMENT() As String
                            DEPARTMENT = STORE.CheckedItems(j).ToString().Split("-->")



                            sqlstring = "insert into indent_auth_list(UserId,UserName,Mobile,Email,Store,Designation,Limit,OrderNo,Status,FromDate,ToDate,Typeofservice,AddUserId,AddDateTime) values('" & txt_userid.Text & "','" & txt_username.Text & "','" & txt_mobileno.Text & "','" & txt_email.Text & "','" & DEPARTMENT(0) & "','" & combo_desig.Text & "','" & txt_limit.Text & "','" & ORDERNO & "','" & combo_status.Text & "','" & fd & "','" & td & "','Indent','" & gUsername & "',getdate())"
                            ReDim Preserve insert(insert.Length)
                            insert(insert.Length - 1) = sqlstring


                            sqlstring = "insert into indent_auth_list(UserId,UserName,Mobile,Email,Store,Designation,Limit,OrderNo,Status,FromDate,ToDate,Typeofservice,AddUserId,AddDateTime) values('" & txt_userid.Text & "','" & txt_username.Text & "','" & txt_mobileno.Text & "','" & txt_email.Text & "','" & DEPARTMENT(0) & "','" & combo_desig.Text & "','" & txt_limit.Text & "','" & ORDERNO & "','" & combo_status.Text & "','" & fd & "','" & td & "','PO','" & gUsername & "',getdate())"
                            ReDim Preserve insert(insert.Length)
                            insert(insert.Length - 1) = sqlstring
                    ...
                    Else
                        For j = 0 To STORE.CheckedItems.Count - 1
                            Dim A = Split(STORE.CheckedItems(i), "-->")
                            sqlstring = "insert into indent_auth_list(UserId,UserName,Mobile,Email,Store,Designation,Limit,OrderNo,Status,FromDate,ToDate,Typeofservice,AddUserId,AddDateTime) values('" & txt_userid.Text & "','" & txt_username.Text & "','" & txt_mobileno.Text & "','" & txt_email.Text & "','" & A & "','" & combo_desig.Text & "','" & txt_limit.Text & "','" & ORDERNO & "','" & combo_status.Text & "','" & fd & "','" & td & "','" & combo_service.Text & "','" & gUsername & "',getdate())"
                            ReDim Preserve insert(insert.Length)
                            insert(insert.Length - 1) = sqlstring
                        Next
                    End If
                ...
                ElseIf (LCase(btn_save.Text) = "update") Then
                    ...
                    For j = 0 To STORE.CheckedItems.Count - 1
                        ...
                        sqlstring = "update indent_auth_list set UserName='" & txt_username.Text & "',Mobile=" & txt_mobileno.Text & ",Email='" & txt_email.Text & "',Store='" & DEPARTMENT(0) & "',Designation='" & combo_desig.Text & "',Limit=" & txt_limit.Text & ",OrderNo=" & txt_orderno.Text & ",Status='" & combo_status.Text & "',FromDate='" & fd & "',ToDate='" & td & "',Typeofservice='" & combo_service.Text & "',UpdateDateTime=getdate(),UpdateUserId='" & gUsername & "' where UserId='" & txt_userid.Text & "' and autoid=" + autoid.ToString() + ""
                        ReDim Preserve insert(insert.Length)
                        insert(insert.Length - 1) = sqlstring
                    Next
                    sqlstring = " SELECT ORDERNO AS ORDERNO FROM indent_auth_list WHERE STORE='" & txt_store.Text & "' AND LIMIT='" & txt_limit.Text & "' and Designation='" & combo_desig.Text & "'"
    ...
    Private Sub load_grid()
        Dim Sql As String
        Try
            Sql = "select UserId,Typeofservice,UserName,Mobile,Email,Store,Designation,Limit,OrderNo,Status,FromDate,ToDate,autoid from indent_auth_list"
    ...
    Public Sub LOADUSERDET()
        ...
        a1 = grid_details.Item(5, grid_details.CurrentRow.Index).Value.ToString()
        For i = 0 To STORE.Items.Count - 1
            a3 = STORE.Items(i).ToString().Split("-->")
            If a1 = a3(0) Then
                STORE.SetItemChecked(i, True)
            End If
        Next
        autoid = grid_details.Item(12, grid_details.CurrentRow.Index).Value.ToString()
    End Sub
    ...
    Private Sub CHK_STORE_CheckedChanged(sender As Object, e As EventArgs) Handles CHK_STORE.CheckedChanged
        Dim i As Integer
        If CHK_STORE.Checked = True Then
            For i = 0 To STORE.Items.Count - 1
                STORE.SetItemChecked(i, True)
            Next
        Else
    ...
    Private Sub STORE_SelectedIndexChanged(sender As Object, e As EventArgs) Handles STORE.SelectedIndexChanged
        If (LCase(btn_save.Text) = "update") Then
            Dim ss As Integer = STORE.CheckedItems.Count - 1
            If ss > 1 Then
