...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.lbl_Heading.Size = New System.Drawing.Size(186, 18)
        Me.lbl_Heading.TabIndex = 18
        Me.lbl_Heading.Text = "AUTO DELIVERY ORDER"
        ...
        Me.Label52.Location = New System.Drawing.Point(18, 21)
        Me.Label52.Name = "Label52"
        Me.Label52.Size = New System.Drawing.Size(71, 14)
        ...
        Me.Label15.Location = New System.Drawing.Point(1, 76)
        ...
        Me.Label38.Location = New System.Drawing.Point(45, 50)
...
#End Region
    ...
    Private Sub GetRights()
        ...
        Dim chstr As String
        SQLSTRING = "SELECT * FROM useradmin WHERE USERNAME = '" & Trim(gUsername) & "' AND MAINGROUP='PURCHASE ORDER' AND MODULENAME LIKE 'Auto Delivery Order%' ORDER BY RIGHTS"
    ...
    Private Sub Cmd_PONoHelp_Click(sender As Object, e As EventArgs) Handles Cmd_PONoHelp.Click
        ...
        If txt_docno.Text <> "" Then
            ...
            M_WhereCondition = "  AUTHDOCNO='" + txt_docno.Text + "' AND DITYPE='AUTO'"
        Else
            gSQLString = "SELECT DISTINCT ISNULL(DINO,'') AS DINO,ISNULL(DIdate,'')AS DIDATE FROM DI_HDR"
            M_WhereCondition = " WHERE DITYPE='AUTO' "
        End If
    ...
    Private Sub cmddochelp_Click(sender As Object, e As EventArgs) Handles cmddochelp.Click
        Try
            gSQLString = "SELECT DISTINCT AUTHDOCNO,DIDATE  FROM  DI_HDR WHERE "
            M_WhereCondition = " DITYPE='AUTO'"
            Dim vform As New ListOperattion1
            vform.Field = "AUTHDOCNO,DIDATE"
    ...
    Private Sub txt_PONo_Validated(sender As Object, e As EventArgs) Handles txt_PONo.Validated
        ...
        Try
            If Trim(txt_PONo.Text) <> "" Then
                sqlstring = "SELECT ISNULL(AUTHDOCNO,'')AUTHDOCNO,ISNULL(STORE,'')STORE,ISNULL(DINO,'')DINO,ISNULL(DIDATE,'')DIDATE,ISNULL(VENDORCODE,'')VENDORCODE,ISNULL(VENDORNAME,'')VENDORNAME,ISNULL(FREEZE,'')FREEZE,ISNULL(UPDATEUSER,'')UPDATEUSER,ISNULL(UPDATETIME,'')UPDATETIME FROM DI_HDR WHERE DINO='" + txt_PONo.Text + "'"
            ...
            sqlstring = "SELECT ISNULL(ITEMCODE,'')ITEMCODE	,ISNULL(ITEMNAME,'')ITEMNAME,	ISNULL(QTY,0)QTY,	UOM	,RATE,	AMOUNT,isnull(FREEZE,'')as FREEZE FROM DI_DET WHERE  DINO ='" & Trim(txt_PONo.Text) & "' ORDER BY AUTOID"
    ...
    Private Sub CmdAdd_Click(sender As Object, e As EventArgs) Handles CmdAdd.Click
        ...
        Dim FromStorecode, ToStorecode, STRITEMQTY, GRIDQTY, CURRENTQTY, CURRENTRATE, CURRENTAMOUNT, Item, VALE As String

        Dim i, j As Integer
        Try
            sqlstring = "select * from di_det where dino='" & txt_PONo.Text & "'"
            gconnection.getDataSet(sqlstring, "DI_DET")
            If gdataset.Tables("DI_DET").Rows.Count > 0 Then
                For i = 1 To gdataset.Tables("DI_DET").Rows.Count
                    STRITEMQTY = Trim(gdataset.Tables("di_det").Rows(j).Item("Qty"))
                    For j = 1 To ssgrid.DataRowCnt
                        ssgrid.Row = j
                        ssgrid.Col = 4
                        GRIDQTY = ssgrid.Text
                        If STRITEMQTY <= Trim(ssgrid.Text) Then
                            MessageBox.Show("Qty Should Not Be Greater Than Current Qty", MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
            ...
            If MessageBox.Show("Do You Want To Update existing Do ", MyCompanyName, MessageBoxButtons.OKCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
                For i = 1 To ssgrid.DataRowCnt
                    With ssgrid
                        ssgrid.Col = 1
                        ssgrid.Row = i
                        ...
                        sqlstring = sqlstring & " SET QTY= '" + CURRENTQTY + "',AMOUNT='" + CURRENTAMOUNT + "',"
        ...
        Try
            If Asc(e.KeyChar) = 13 Then
                If Trim(txt_docno.Text) = "" Then
                    Call cmddochelp_Click(cmddochelp, e)
                Else
                    txt_PONo_Validated(txt_docno, e)
                End If
            End If
        Catch ex As Exception
            MessageBox.Show("Plz Check Error : " & ex.Message, MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
